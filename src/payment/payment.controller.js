  ...
  static async createPlans () {
    try {
      ...
      for (const plan of stripePlans.data) {
        ...
        try {
          ...
          console.log('Skiping plan [- %s -]', plan.id)
        }
      }
      await timer(400)
      ...
      for (const product of stripeProducts.data) {
        ...
        try {
          ...
          console.log('Skiping product [- %s -]', product.id)
        }
      }
      await timer(400)
    ...
    static async dropAllSubscriptions () {
      try {
        ...
        // -- Retrieve and delete
        subscriptions = await StripeAPI.retrieveAllSubscriptions()
     ...
     */
    static async createAndUpdateSubscription (data = {
      token: String,
      email: String,
      amount: Number,
      currency: String,
      ...
      try {
        ...
        if (!subscription) {
          ...
          if (user.customerId === null || user.customerId === undefined || user.customerId.length <= 0) {
    ...
    static async deleteSubscription (conversationId) {
      try {
        ...
        console.log('PaymentController (deleteSubscription) error :: ', reason)
        throw new Error(reason)
      }
    }
}
