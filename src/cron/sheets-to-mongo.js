...
class SheetsToMongo {
  ...
  static async execute () {
    ...
    try {
      ...
      const formatted = [].concat.apply([], response.map((rows, index) => {
        return [].concat.apply([], rows.map((document, id) => {
          return [].concat(
            ...
            basicSender.prepareContent('', '', '', document.listen_link),
                ...
                .filter(content => {
                  return (content.message.type !== undefined && content.message.content !== undefined)
                }).map((content, _index_) => {
              let type = 'week'
